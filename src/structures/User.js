...
class User extends Base {
  ...
  _patch(data) {
    ...
    /**
     * @typedef {Object} ProfileEffectData
     * @property {Snowflake} skuId The id of the profile effect's SKU
     * @property {string} asset The profile effect's asset path
     * @property {string} label The profile effect's label
     * @property {?string} reducedMotionAsset The reduced motion version of the profile effect
     * @property {?number} animationType The animation type of the profile effect
     * @property {?string} description The description of the profile effect
     * @property {?Date} expiresAt When the profile effect expires (for temporary effects)
     */

    /**
     * @typedef {Object} Collectibles
     * @property {?NameplateData} nameplate The user's nameplate data
     * @property {?ProfileEffectData} profileEffect The user's profile effect data
     */
    ...
    if (data.collectibles) {
      ...
      if (data.collectibles.profile_effect) {
        this.collectibles.profileEffect = {
          skuId: data.collectibles.profile_effect.sku_id,
          asset: data.collectibles.profile_effect.asset,
          label: data.collectibles.profile_effect.label,
          reducedMotionAsset: data.collectibles.profile_effect.reduced_motion_asset ?? null,
          animationType: data.collectibles.profile_effect.animation_type ?? null,
          description: data.collectibles.profile_effect.description ?? null,
          expiresAt: data.collectibles.profile_effect.expires_at
            ? new Date(data.collectibles.profile_effect.expires_at)
            : null,
        };
      } else {
        this.collectibles.profileEffect = null;
      }
    } else {
  ...
  /**
   * A link to the user's profile effect.
   * @param {boolean} [reducedMotion=false] Whether to get the reduced motion version
   * @returns {?string}
   */
  profileEffectURL(reducedMotion = false) {
    if (!this.collectibles?.profileEffect) return null;
    const asset =
      reducedMotion && this.collectibles.profileEffect.reducedMotionAsset
        ? this.collectibles.profileEffect.reducedMotionAsset
        : this.collectibles.profileEffect.asset;
    if (!asset) return null;
    return this.client.rest.cdn.ProfileEffect(asset);
  }

  /**
   * A link to the user's nameplate.
   * @returns {?string}
   */
  nameplateURL() {
    if (!this.collectibles?.nameplate) return null;
    return this.client.rest.cdn.Nameplate(this.collectibles.nameplate.asset);
  }

  /**
   * Check if the user's profile effect is expired
   * @returns {boolean}
   */
  get isProfileEffectExpired() {
    if (!this.collectibles?.profileEffect?.expiresAt) return false;
    return this.collectibles.profileEffect.expiresAt < new Date();
  }

  /**
   * Get all collectibles URLs
   * @returns {Object} Object containing all collectible URLs
   */
  getCollectiblesURLs() {
    return {
      profileEffect: this.profileEffectURL(),
      profileEffectReducedMotion: this.profileEffectURL(true),
      nameplate: this.nameplateURL(),
      avatarDecoration: this.avatarDecorationURL(),
      guildTagBadge: this.guildTagBadgeURL(),
    };
  }
